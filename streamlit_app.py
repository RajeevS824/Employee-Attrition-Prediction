...
# ==========================================================
    ...
    # correlation plot
    st.subheader("📊 Top Features Correlated with Attrition")
    numeric_df = df.select_dtypes(include=['int64','float64'])
    corr = numeric_df.corr()['Attrition'].drop('Attrition').sort_values(key=abs, ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(6,4))
    corr.plot(kind='barh', ax=ax, title="Top Correlated Features with Attrition")
    st.pyplot(fig)

    features = ['OverTime','MaritalStatus','DistanceFromHome','JobRole','Department',
                'TotalWorkingYears','JobLevel','YearsInCurrentRole','MonthlyIncome',
                'Age','YearsWithCurrManager','StockOptionLevel','YearsAtCompany','JobInvolvement']
    X = df[features]
    y = df['Attrition']
    ...
    user_input = {}
    for f in features:
        if f in ['OverTime','MaritalStatus','JobRole','Department']:
        ...
        elif f in ['Age','DistanceFromHome','YearsAtCompany',
                   'YearsInCurrentRole','YearsWithCurrManager','TotalWorkingYears']:
            user_input[f] = st.number_input(f, min_value=0, max_value=50, step=1)
        else:  # MonthlyIncome etc.
            user_input[f] = st.number_input(f, min_value=0, step=100)
...
# correlation plot
    st.subheader("📊 Top Features Correlated with Performance Rating")
    numeric_df = df.select_dtypes(include=['int64','float64'])
    corr = numeric_df.corr()['PerformanceRating'].drop('PerformanceRating').sort_values(key=abs, ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(6,4))
    corr.plot(kind='barh', ax=ax, title="Top Correlated Features with Performance Rating")
    st.pyplot(fig)

    features = ['YearsInCurrentRole','YearsWithCurrManager','YearsSinceLastPromotion',
    ...
    user_input = {}
    for f in features:
        if f in ['RelationshipSatisfaction','EnvironmentSatisfaction','JobInvolvement']:
            user_input[f] = st.slider(f, 1, 4, 2)
        elif f == 'PercentSalaryHike':   
            user_input[f] = st.slider("Percent Salary Hike (%)", 0, 50, 10)
        elif f in ['DistanceFromHome','YearsSinceLastPromotion','YearsInCurrentRole',
                   'YearsWithCurrManager','TotalWorkingYears']:
            user_input[f] = st.number_input(f, min_value=0, max_value=50, step=1)
        else:
            user_input[f] = st.number_input(f, min_value=0, step=1)
